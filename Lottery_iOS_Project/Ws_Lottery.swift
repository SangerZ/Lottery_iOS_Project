...
class Ws_Lottery {
    ...
    // GetLottery
    static func GetLottery(period_lottery_date: String,user_id: String,completion:(responseData:mLotteryPeriod,errorMessage:NSError?)->Void)
    {
        ...
        let task = NSURLSession.sharedSession().dataTaskWithURL(url!) {(data, response, error) in
            do {
                ...
                if let arrayOfDictionaries = jsonResult as? AnyObject {
                    lotteryPeriod = mLotteryPeriod(dictionary: arrayOfDictionaries )
